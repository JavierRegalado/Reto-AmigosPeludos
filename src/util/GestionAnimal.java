...
public class GestionAnimal {
	...
	public void añadirAnimal(Animal ani) throws SQLException {
		String insert = "INSERT INTO animales (id_animal, id_centro, nombre, edad_meses, especie, caracteristicas, necesidades, tamano, entrenado, sociabilidad, pelaje, color, agua_dulce) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
	...
	public Animal modAnimal(Animal ani) throws SQLException {
		String update = "UPDATE animales SET id_centro = ?, nombre = ?, edad_meses = ?, caracteristicas = ?, necesidades = ?, tamano = ?, entrenado = ?, sociabilidad = ?, pelaje = ?, color = ?, agua_dulce = ? WHERE id_animal = ?";
	...
	// Lo creamos para no repetir el código de sacar datos del ResultSet en Buscar y
	// en LeerTodos
